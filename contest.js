    ...
    data.forEach(element => {
        ...
        if (Number((element.start_time).slice(14, 16)) + 30 === 60) {
            if (Number((element.end_time).slice(14, 16)) + 30 === 60) {
                time = `${Number((element.start_time).slice(11, 13)) + 6}:00 to ${Number((element.end_time).slice(11, 13)) + 6}:00`
            }
            else {
            ...
            if (Number((element.end_time).slice(14, 16)) + 30 === 60) {
                time = `${Number((element.start_time).slice(11, 13)) + 5}:00 to ${Number((element.end_time).slice(11, 13)) + 6}:00`
            }
            else {
                time = `${Number((element.start_time).slice(11, 13)) + 5}:00 to ${Number((element.end_time).slice(11, 13)) + 5}:${Number((element.end_time).slice(14, 16)) + 30}`
            }
        }
        let duration = `${Number(element.duration) / 3600} hr: ${Number(element.duration) % 60} min`
        contestDetail.innerHTML += "<div class='segment'><div class='data'><div class='name'> <span>Contest Name: </span>" + name + "</div><div class='url'>Contest Link: " + url + "</div><div class='date'>Date: " + date + "</div><div class='time'>Time: " + time + "</div><div class='duration'>Duration: " + duration + "</div></div>"
    })
    console.log(data);
}
